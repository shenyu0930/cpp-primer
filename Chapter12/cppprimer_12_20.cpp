...
public:
	...
	StrBlobPtr& incr();
	std::size_t getCurr() { return curr; }
private:
...
int main()
{
	ifstream input("D:/C++test/leetcode_test/cppprimer_12_21.txt");
	string str;
	StrBlob strBlob;
	...
	for (StrBlobPtr pbeg(strBlob.begin()), pend(strBlob.end()); pbeg.getCurr() != pend.getCurr();
		pbeg.incr())
